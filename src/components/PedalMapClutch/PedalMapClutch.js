...
import pedalMapClutchPositionFilter from "./pedalMapClutchPositionFilter";
...
const PedalMapClutch = () => {
    ...
    useEffect(async ()=>{
        ...
        if(connected && visible){
            ...
            getStream
                ...
                .pipe(map(message => pedalMapClutchPositionFilter(message)))
                .subscribe({
                    next: (message) => {
                        adddata({clutch : message.clutch})
                    },
                    complete: () => {
    ...
    function updateMap({clutchMap}){
        chartInstance.data.datasets[1].data = clutchMap
        chartInstance.update();
    }

    function adddata({clutch}){
